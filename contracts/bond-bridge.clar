...
(define-constant ERR_UNKNOWN_ANNOUNCEMENT (err u4002))
(define-constant ERR_ADDRESS_ANNOUNCED (err u4003))
(define-constant ERR_DEPOSIT_NOT_SWEPT (err u4004))
(define-constant ERR_DEPOSIT_MISDIRECTED (err u4005))
;; u4006 retired (ERR_DEPOSIT_TOO_SMALL): a short deposit is credited for what
;; arrived. Number kept reserved.
(define-constant ERR_DEPOSIT_CREDITED (err u4007))
(define-constant ERR_ANNOUNCEMENT_LIVE (err u4008))
(define-constant ERR_UNKNOWN_WITHDRAWAL (err u4009))
(define-constant ERR_WITHDRAWAL_PENDING (err u4010))
(define-constant ERR_UNKNOWN_COMMITMENT (err u4012))
(define-constant ERR_COMMITMENT_EXISTS (err u4013))
(define-constant ERR_REVEAL_TOO_SOON (err u4014))
(define-constant ERR_UNSUPPORTED_ADDRESS (err u4015))
(define-constant ERR_TXID_MISMATCH (err u4016))
(define-constant ERR_MALFORMED_TX (err u4017))
(define-constant ERR_INPUT_COUNT (err u4018))
(define-constant ERR_PARENT_MISMATCH (err u4019))
(define-constant ERR_FOREIGN_INPUT (err u4020))
(define-constant ERR_ANNOUNCED_TOO_LATE (err u4021))
(define-constant ERR_UNPROVABLE_ADDRESS (err u4022))
(define-constant ERR_WRONG_KEY (err u4023))
(define-constant ERR_BAD_SIGNATURE (err u4024))

;; Error codes carried through the `check-input` fold (no `err` from a fold).
(define-constant CHECK_OK u0)
(define-constant CHECK_MALFORMED u4017)
(define-constant CHECK_PARENT u4019)
(define-constant CHECK_FOREIGN u4020)
