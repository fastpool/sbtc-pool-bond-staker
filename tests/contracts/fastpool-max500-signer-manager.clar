...
;;
;; Mainnet `SPMPMA1V6P430M8C91QS1G9XJ95S59JS1TZFZ4Q4.fastpool-max500-signer-manager`,
;; published at block 8674707 -- the signer manager this pool
;; is built to stake through, and so the one its tests stake through.
;;
;; Exactly one mechanical change from the published source: the 9 occurrences
;; of the pox-5 boot address `SP000000000000000000002Q6VF78` are rewritten
;; to `ST000000000000000000002AMW42H`, which is where simnet boots it. That is
...
(define-public (settle-staker-rewards
  ...
  (let (
    ...
    (var-set total-unclaimed-rewards (- (var-get total-unclaimed-rewards) gross))
